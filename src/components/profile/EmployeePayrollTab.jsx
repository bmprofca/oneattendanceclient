...
import { FaCalendarAlt, FaCheckCircle, FaChevronDown, FaEye, FaMoneyBillWave, FaSpinner } from "react-icons/fa";
...
export default function EmployeePayrollTab({ employeeId, refreshKey = 0 }) {
  ...
  const fetchPayroll = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !result.success) throw new Error(result.message || "Failed to load payroll records");
      ...
      updatePagination({ page: 1, total: nextRecords.length, limit: pagination.limit });
    } catch (error) {
      setRecords([]);
  ...
  const visibleRecords = useMemo(() => {
    ...
    return records.slice(start, start + pagination.limit);
  }, [pagination.page, pagination.limit, records]);
  ...
  const cardRenderer = (record, index) => {
    const payroll = record.payroll || {};
    return (
      <ManagementCard key={`${payroll.id || payroll.payroll_period || index}`} accent="emerald" delay={index * 0.04} onClick={() => setSelectedRecord(record)} activeId={activeActionMenu} onToggle={(event, id) => setActiveActionMenu((current) => current === id ? null : id)} menuId={`payroll-${payroll.id || index}`} actions={actions(record)} hoverable title={payroll.month && payroll.year ? `${payroll.month}/${payroll.year}` : formatDate(payroll.payroll_period)} subtitle={`${record.status || "current"} payroll`} eyebrow="Payroll Record" badge={<span className="text-xs font-bold text-emerald-700">#{payroll.id || "Preview"}</span>} footer={<div className="flex w-full justify-between text-xs text-slate-400"><span>Net: {formatCurrency(payroll.net_salary)}</span><span>Worked: {formatMinutes(payroll.work?.worked_minutes)}</span></div>}>
        <div className="grid grid-cols-3 gap-2">
          <div className="rounded-lg border border-emerald-100 bg-emerald-50 p-2 text-center"><p className="text-[9px] font-bold uppercase text-emerald-500">Earnings</p><p className="text-xs font-black text-emerald-700">{formatCurrency(payroll.total_earnings)}</p></div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between rounded-xl bg-white p-2 shadow-lg">
        <p className="flex items-center gap-2 px-4 text-md font-semibold text-emerald-700"><FaMoneyBillWave size={12} />Payroll</p>
        {records.length > 0 && <ManagementViewSwitcher viewMode={viewMode} onChange={setViewMode} accent="emerald" />}
      </div>
      {loading ? <div className="flex flex-col items-center gap-2 py-10 text-slate-400"><FaSpinner className="animate-spin text-emerald-600" /><span className="text-sm">Loading payroll records...</span></div> : records.length === 0 ? <div className="py-12 text-center text-sm text-slate-400">No payroll records found</div> : viewMode === "table" ? <ManagementTable rows={visibleRecords} columns={columns} rowKey={(record, index) => record.payroll?.id || `${record.status}-${index}`} onRowClick={setSelectedRecord} activeId={activeActionMenu} onToggleAction={(event, id) => setActiveActionMenu((current) => current === id ? null : id)} getActions={actions} accent="emerald" /> : <ManagementGrid viewMode={viewMode}>{visibleRecords.map(cardRenderer)}</ManagementGrid>}
      {!loading && records.length > 0 && <Pagination currentPage={pagination.page} totalItems={records.length} itemsPerPage={pagination.limit} onPageChange={goToPage} onLimitChange={changeLimit} showInfo />}
